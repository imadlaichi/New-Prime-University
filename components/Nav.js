...
backdrop-blur-sm text-3xl xl:text-xl xl:rounded-full '>
      {navData.map((link, index)=>{
        return( <Link className={`${link.path === pathname && "text-primary "} relative flex items-center group hover:text-primary transition-all duration-300`} href={link.path} key={index}>
